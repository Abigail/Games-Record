...
#
# May not drop out of bounds
#
my $EXTRA = 3;
for (my $x = 0; $x < $X_SIZE + $EXTRA; $x ++) {
    for (my $y = 0; $y < $Y_SIZE + $EXTRA; $y ++) {
        next if $x < $X_SIZE && $y < $Y_SIZE;
        my $res = $game -> _is_valid_move
                                    (move => [$MOVE_TYPE_DROP => [$x, $y]]);
        ok !$res, "May not drop ($x, $y) because it's outside of the board";
        is  $game -> error, $MOVE_ERROR_FIELD_DOES_NOT_EXIST,
                            "Error 'Field does not exist' was set";
    }
}
...
for (my $x = 0; $x < $X_SIZE; $x ++) {
    for (my $y = 0; $y < $Y_SIZE; $y ++) {
        my $res = $game -> _is_valid_move
                                    (move => [$MOVE_TYPE_DROP => [$x, $y]]);
        ok !$res, "May not drop on filled board";
        is  $game -> error, $MOVE_ERROR_FIELD_OCCUPIED,
                            "Error 'Field occupied' was set";
    }
}
